...
```mermaid
graph LR
    subgraph Container ["CHỌN NGUYÊN TỐ CỦA BẠN (CHOOSE YOUR ELEMENT)"]
        direction LR
        Sc["[Sc]<br/>SCHOOL<br/>Scholar"]
        Hm["[Hm]<br/>HOME<br/>Recovery"]
        Ki["[Ki]<br/>SPORT<br/>Kinetic"]
        Ba["[Ba]<br/>BASE<br/>Daily"]
    end

    style Sc fill:#fff,stroke:#333,stroke-width:1px
    style Hm fill:#fff,stroke:#333,stroke-width:1px
    style Ki fill:#fff,stroke:#333,stroke-width:1px
    style Ba fill:#fff,stroke:#333,stroke-width:1px
```
...
```mermaid
graph TD
    subgraph FilterPanel ["BỘ LỌC (FILTER BY)"]
        Cat["Danh Mục:<br/>☑ Polo Shirts (18)<br/>☐ Dress Shirts (12)<br/>☐ Shorts (8)"]
        Size["Kích Thước:<br/>☐ S ☐ M ☑ L ☐ XL"]
        Color["Màu Sắc:<br/>🟨 Lab White (12) 🟦 Sky Blue (8)"]
        Tech["Công Nghệ Vải:<br/>☐ CVC Pique<br/>☐ Coolmax"]
        Price["Khoảng Giá:<br/>150.000đ - 800.000đ"]
        Actions["[ĐẶT LẠI] [ÁP DỤNG]"]
    end
```
...
```mermaid
graph LR
    subgraph Left ["TRÌNH CHIẾU MEDIA (60%)"]
        Viewer["TRÌNH XEM SẢN PHẨM 3D<br/>(Tương tác 360°)"]
        Controls["[Xoay] [Phóng to] [Xem Vải]"]
        Gallery["Thư viện ảnh nhỏ (Thumbnails)"]
    end

    subgraph Right ["THÔNG TIN & MUA HÀNG (40%)"]
        Header["THE LAB TEE<br/>289.000 ₫<br/>(Element: Ba)"]
        Selectors["SIZE: S M L XL<br/>MÀU: Bảng màu"]
        Quantity["SỐ LƯỢNG: - 1 +"]
        Actions["🛒 THÊM VÀO GIỎ<br/>♡ LƯU LẠI"]
        Trust["✓ Tồn kho: 124<br/>📦 Giao: 2-3 ngày<br/>↻ Đổi trả 30 ngày"]
    end
...
```mermaid
graph TD
    Header["Header Cố định (70px)"]
    Hero["Hero Section (100vh)<br/>Video nền + Text động"]
    Collections["Featured Collections<br/>Cuộn ngang"]
    Philosophy["Brand Philosophy (60vh)<br/>Chia đôi màn hình"]
    Grid["Best Sellers Grid<br/>Lưới 4 cột"]
    Showcase["3D Showcase (80vh)<br/>Sticky Scroll"]
    Footer["Footer"]

    Header --> Hero --> Collections --> Philosophy --> Grid --> Showcase --> Footer
```
...
```mermaid
stateDiagram-v2
    [*] --> StaticNav: Trang chưa cuộn
    StaticNav --> AtomicLogo: Cuộn xuống > 50px

    state AtomicLogo {
        [*] --> Collapsed: Logo thu gọn (16x16)
        Collapsed --> Expanded: Hover
        Expanded --> Collapsed: Mouse Leave
        Expanded: Menu items fly-out với blur effect
    }

    AtomicLogo --> StaticNav: Cuộn lên đầu trang
```
